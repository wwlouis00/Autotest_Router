...
def replace_chromedriver(directory):
    """只替換 'SkuTool' 目錄內的 ChromeDriver，避免所有子目錄都放置 ChromeDriver."""
    try:
        ...
        # **只處理符合 'YYYYMMDD SkuTool' 的資料夾**
        version_folder_pattern = r"\d{8} SkuTool"
        ...
        for folder_name in os.listdir(directory):
            ...
            if not re.match(version_folder_pattern, folder_name):
                logger.debug(f"Skipping non-SkuTool folder: {folder_name}")
                continue  # 只處理符合 `YYYYMMDD SkuTool` 格式的資料夾
            ...
            try:
                if os.path.exists(chromedriver_path):
                    ...
                    logger.info(f"Removed old ChromeDriver: {chromedriver_path}")

                shutil.copy(os.path.join(CHROMEDRIVER_FOLDER, CHROMEDRIVER_FILE_test), chromedriver_path)
                logger.info(f"Replaced ChromeDriver in: {chromedriver_path}")

            except Exception as e:
                logger.error(f"Failed to replace ChromeDriver at {chromedriver_path}: {e}")
...
def download_and_extract(ftp, local_folder):
    ...
    def download_file(ftp, remote_filename, local_path):
        ...
        if not os.path.exists(local_path) or os.path.getsize(local_path) < 1024:
            logger.error(f"Download failed or file is too small: {local_path}")
            os.remove(local_path)

        logger.info(f"Download completed: {local_path}")

    def extract_file(local_path, extract_path):
        """解壓 7z 文件到指定文件夾，並確認重要文件是否存在。"""
    ...
    try:
        ...
        for filename in filenames:
            ...
            if is_valid_extracted(extract_path):
                ...
                logger.error(f"Extraction failed or missing files in {extract_path}")
...
def main():
    ...
    # 收件人列表框架
    frame_recipients = tk.Frame(root, bg="#222222", bd=2, relief=tk.GROOVE)
    frame_recipients.pack(pady=10, padx=10, fill=tk.BOTH, expand=False)
    ...
    recipient_list_frame = tk.Frame(frame_recipients, bg="#333333")
