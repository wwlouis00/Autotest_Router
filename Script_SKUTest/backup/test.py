...
def start_process(start_button,stop_button,local_folder, selected_value):
    """發送郵件給所有收件人"""
    recipients = [email.strip() for email in recipient_entries if email.strip()]
    
    if selected_value == 0:
        messagebox.showwarning("警告", "請選擇至少一個測試項目！")
        return

    if not recipients:
        messagebox.showerror("錯誤", "請輸入至少一個收件人！")
        return  # 直接返回，避免繼續執行
    logger.info(f"執行測試: {selected_value}")
...
#     print(f"選擇的選項值: {selected_value}")  # 這裡可以換成 logger 記錄
...
def create_zip_and_send_email(gateway_ip,recipients):
   # 檢查所需檔案是否存在
    ...
    if found_file_name:
        ...
        zip_file_name = f"SKU Test Result Report - {found_file_name} {CURRENT_TIME}" + '.zip'
...
def main():
    ...
    start_button = tk.Button(
        ...
        command=lambda: start_process(start_button, stop_button,local_folder, selected_option.get())
    )
